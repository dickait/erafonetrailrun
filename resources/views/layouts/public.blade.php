    ...
    <nav id="main-nav" class="fixed top-0 left-0 right-0 z-50 bg-white border-b border-surface-300 shadow-sm">
                ...
                <div class="hidden md:flex items-center gap-4">
                    ...
                    <div class="relative group">
                        ...
                        <div class="absolute right-0 top-full mt-1 w-36 bg-white border border-surface-300 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 overflow-hidden z-50">
                            ...
                            <a href="{{ route('lang.switch', 'en') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm {{ app()->getLocale() === 'en' ? 'text-brand-500 bg-brand-50' : 'text-surface-600 hover:bg-surface-100' }} transition-colors">
                                🇬🇧 English
                            </a>
                        </div>
                    </div>
        ...
        <div id="mobile-menu" class="hidden md:hidden border-t border-surface-300 bg-white/95 backdrop-blur-xl">
            <div class="px-4 py-4 space-y-2">
                ...
                @auth
                    <a href="{{ route('dashboard') }}" class="block px-4 py-2.5 rounded-lg text-sm font-medium text-surface-800 hover:text-brand-500 hover:bg-surface-100 transition-colors">{{ __('messages.nav_dashboard') }}</a>
                @else
                    <a href="{{ route('login') }}" class="block px-4 py-2.5 rounded-lg text-sm font-medium text-surface-800 hover:text-brand-500 hover:bg-surface-100 transition-colors">{{ __('messages.nav_login') }}</a>
                @endauth
                <!-- Mobile Language Switcher -->
                <div class="flex gap-2 px-4 py-2">
                    <a href="{{ route('lang.switch', 'id') }}" class="px-3 py-1.5 text-xs rounded-lg {{ app()->getLocale() === 'id' ? 'bg-brand-50 text-brand-500' : 'text-surface-500 hover:bg-surface-100' }}">🇮🇩 ID</a>
                    <a href="{{ route('lang.switch', 'en') }}" class="px-3 py-1.5 text-xs rounded-lg {{ app()->getLocale() === 'en' ? 'bg-brand-50 text-brand-500' : 'text-surface-500 hover:bg-surface-100' }}">🇬🇧 EN</a>
                </div>
                <a href="{{ route('register.create') }}" class="block px-4 py-2.5 rounded-lg text-sm font-semibold text-white bg-gradient-to-r from-brand-500 to-brand-600 text-center">{{ __('messages.nav_register') }}</a>
            </div>
        </div>
    </nav>
    ...
    <footer class="bg-surface-900 border-t border-surface-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div>
                    <h4 class="font-display font-semibold text-sm uppercase tracking-wider text-accent-400 mb-4">{{ __('messages.footer_quick_links') }}</h4>
                    <ul class="space-y-2">
                ...
                <div>
                    <h4 class="font-display font-semibold text-sm uppercase tracking-wider text-accent-400 mb-4">{{ __('messages.footer_contact') }}</h4>
                    <ul class="space-y-2">
