    ...
    <nav id="main-nav" class="fixed top-0 left-0 right-0 z-50 bg-white border-b border-surface-300 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16 md:h-20">
                <a href="{{ route('home') }}" class="flex items-center gap-3 group">
                    <img src="{{ asset('logo.png') }}" class="h-10 md:h-12 w-auto" alt="ERA TRAIL RUN" />
                </a>
                ...
                <div class="hidden md:flex items-center gap-4">
                    ...
                    @else
                        <a href="{{ route('login') }}"
                            class="text-sm font-medium text-surface-800 hover:text-brand-500 transition-colors">{{ __('messages.nav_login') }}</a>
                    @endauth
        ...
        <div id="mobile-menu" class="hidden md:hidden border-t border-surface-300 bg-white/95 backdrop-blur-xl">
            <div class="px-4 py-4 space-y-2">
                ...
                @else
                    <a href="{{ route('login') }}"
                        class="block px-4 py-2.5 rounded-lg text-sm font-medium text-surface-800 hover:text-brand-500 hover:bg-surface-100 transition-colors">{{ __('messages.nav_login') }}</a>
                @endauth
