    ...
    <div class="bg-white border border-surface-300 rounded-xl overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody>
                @forelse($payments as $pay)
                    <tr class="border-b border-surface-300/50 hover:bg-surface-50/30">
                        <td class="px-4 py-3 font-mono text-xs text-surface-700">
                            {{ $pay->invoice_id ?? $pay->mayar_invoice_id ?? '-' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-surface-700">{{ __('messages.admin_no_payments') }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
        ...
        <div
            ...
            <form id="editForm" method="POST">
                ...
                <div style="display:flex; gap:12px; margin-bottom:16px;">
                    <div style="flex:1;">
                        <label style="display:block; font-size:14px; font-weight:600; color:#333; margin-bottom:6px;">Tanggal Bayar</label>
                        <div style="display:flex; gap:6px; align-items:center;">
                            <select id="modalDay" style="flex:1; padding:10px 6px; border:1px solid #d1d5db; border-radius:10px; font-size:14px; box-sizing:border-box;">
                                @for($d = 1; $d <= 31; $d++)
                                    <option value="{{ str_pad($d, 2, '0', STR_PAD_LEFT) }}">{{ str_pad($d, 2, '0', STR_PAD_LEFT) }}</option>
                                @endfor
                            </select>
                            <span style="font-weight:700; color:#555;">/</span>
                            <select id="modalMonth" style="flex:1.2; padding:10px 6px; border:1px solid #d1d5db; border-radius:10px; font-size:14px; box-sizing:border-box;">
                            ...
                            <span style="font-weight:700; color:#555;">/</span>
                            <select id="modalYear" style="flex:1.4; padding:10px 6px; border:1px solid #d1d5db; border-radius:10px; font-size:14px; box-sizing:border-box;">
                                @for($y = 2024; $y <= 2030; $y++)
                    ...
                    <div style="flex:1;">
                        <label style="display:block; font-size:14px; font-weight:600; color:#333; margin-bottom:6px;">Jam
