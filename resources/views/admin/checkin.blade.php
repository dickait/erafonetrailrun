...
async function doCheckin() {
    ...
    try {
        ...
        if (data.success) {
            result.innerHTML = `
                <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-6 text-center animate-pulse">
                    <svg class="w-16 h-16 mx-auto text-emerald-500 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    <h3 class="font-display font-bold text-xl text-emerald-600 mb-2">{{ __('messages.admin_checkin_success') }}</h3>
                    <p class="text-surface-900 font-medium text-lg">${data.participant.full_name}</p>
                    <p class="text-surface-700 text-sm">BIB: ${data.participant.bib_number || '-'} • ${data.participant.category?.name || ''}</p>
                </div>`;
        } else {
            result.innerHTML = `
                <div class="bg-brand-50 border border-brand-200 rounded-xl p-6 text-center">
                    ...
                    <p class="text-surface-700">${data.message}</p>
                </div>`;
        }
    } catch (e) {
        result.innerHTML = `<div class="bg-brand-50 border border-brand-200 rounded-xl p-6 text-center"><p class="text-red-400">Error: ${e.message}</p></div>`;
    }
