...
@section('title', __('messages.nav_login') . ' - Era Trail Run')
@section('content')
    <section class="pt-28 pb-20 bg-surface-50 min-h-screen flex items-center">
        <div class="max-w-md mx-auto w-full px-4 sm:px-6">
            <div class="text-center mb-8">
                <div
                    class="w-16 h-16 mx-auto bg-gradient-to-br from-brand-500 to-brand-700 rounded-2xl flex items-center justify-center shadow-lg shadow-brand-500/20 mb-4">
            ...
            <form method="POST" action="{{ route('login') }}"
                ...
                <div>
                    <label for="email"
                        class="block text-sm font-medium text-surface-800 mb-1.5">{{ __('messages.login_email') }}</label>
                ...
                <div>
                    <label for="password"
                        class="block text-sm font-medium text-surface-800 mb-1.5">{{ __('messages.login_password') }}</label>
                ...
                <div class="flex items-center justify-between">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="remember"
                            class="w-4 h-4 rounded border-surface-300 bg-surface-50 text-brand-500 focus:ring-brand-500 focus:ring-offset-0">
                        <span class="text-sm text-surface-700">{{ __('messages.login_remember') }}</span>
                    </label>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}"
                            class="text-sm text-brand-500 hover:text-brand-600 transition-colors">{{ __('messages.login_forgot') }}</a>
                    @endif
                </div>
                <button type="submit"
                    class="w-full py-3.5 bg-gradient-to-r from-brand-500 to-brand-600 hover:from-brand-600 hover:to-brand-700 text-white font-bold rounded-xl shadow-lg shadow-brand-500/25 hover:shadow-brand-500/40 transition-all duration-200">
