...
<section class="pt-28 pb-20 bg-surface-50 min-h-screen">
    <div class="max-w-2xl mx-auto px-4 sm:px-6">
        <div class="text-center mb-10">
            <span class="inline-block px-4 py-1.5 bg-brand-50 text-brand-500 text-sm font-semibold rounded-full mb-6 tracking-wide uppercase">{{ __('messages.part_payment_title') }}</span>
            <h1 class="font-display font-bold text-3xl md:text-4xl text-surface-900 mb-2">Complete Your Payment</h1>
            <p class="text-surface-700">Please review your registration details and proceed to payment to secure your spot.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-surface-300 p-6 shadow-sm">
            ...
            <div class="space-y-4">
                ...
                $amount = $participant->latestPayment ? $participant->latestPayment->amount : ($participant->category ? $participant->category->getCurrentPrice() : 0);
                @endphp
                ...
                <div class="flex justify-between py-2 border-b border-surface-100">
                    <span class="text-surface-700 text-sm font-semibold">Total Payment</span>
                    <span class="text-brand-600 text-lg font-bold">Rp {{ number_format($amount, 0, ',', '.') }}</span>
                </div>
            ...
            <div class="mt-8">
                <a href="{{ $participant->latestPayment->payment_link ?? '#' }}" target="_blank" class="block w-full py-4 text-center bg-gradient-to-r from-brand-500 to-brand-600 hover:from-brand-600 hover:to-brand-700 text-white font-bold text-lg rounded-xl shadow-md transition-all">Proceed to Payment (Mayar.id)</a>
            </div>
            @endif
            
            @if($participant->payment_status == 'paid')
            <div class="mt-8">
                <a href="{{ route('registration.status', ['email' => $participant->email]) }}" class="block w-full py-3 text-center bg-surface-100 hover:bg-surface-200 text-surface-900 font-semibold rounded-xl transition-all">Check Status</a>
            </div>
            @endif
        </div>
        @else
        <div class="bg-white rounded-2xl border border-brand-100 p-8 text-center shadow-sm">
            ...
            <a href="{{ route('home') }}" class="mt-6 inline-block px-6 py-2 bg-brand-50 text-brand-600 font-semibold rounded-lg hover:bg-brand-100 transition-colors">Back to Home</a>
        </div>
        @endif
    </div>
</section>
