        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8 border-b border-surface-150 pb-6">
                ...
                <div class="flex items-center gap-2" x-show="activeTab !== 'podium'">
                    <button @click="showAdvanced = !showAdvanced" 
                            :class="showAdvanced ? 'bg-brand-50 border-brand-500 text-brand-600' : 'bg-white border-surface-200 text-surface-700 hover:bg-surface-50'"
            ...
            <div x-show="showAdvanced && activeTab !== 'podium'" 
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    <!-- Distance Filter -->
                    <div>
                        <label class="block text-[10px] font-bold text-surface-400 uppercase tracking-wider mb-1.5">Distance</label>
                        <select x-model="filterDistance" 
                                @change="if(filterDistance !== 'all') { activeTab = filterDistance; }"
                                class="w-full px-3 py-2 bg-white border border-surface-300 rounded-xl text-surface-800 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-colors outline-none text-xs">
                    ...
                    <!-- Gender Filter -->
                    <div>
                        <label class="block text-[10px] font-bold text-surface-400 uppercase tracking-wider mb-1.5">Gender</label>
                    ...
                    <!-- Age Category Filter -->
                    <div>
                        <label class="block text-[10px] font-bold text-surface-400 uppercase tracking-wider mb-1.5">Age Category</label>
                    ...
                    <!-- Sort By Filter -->
                    <div>
                        <label class="block text-[10px] font-bold text-surface-400 uppercase tracking-wider mb-1.5">Sort By</label>
                        <select x-model="sortBy" 
                            ...
                            <option value="pace_asc">Pace (Fastest)</option>
                            <option value="pace_desc">Pace (Slowest)</option>
                        </select>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-surface-200 overflow-hidden shadow-sm min-h-[400px]" x-show="activeTab !== 'podium'">
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs">
                        <thead class="bg-surface-50 text-surface-500 uppercase text-[9px] font-bold tracking-wider border-b border-surface-200">
                            <tr>
                        ...
                        <template x-for="result in filteredResults" :key="result.id">
                            <tbody class="divide-y divide-surface-100 border-b border-surface-100 last:border-b-0">
                                <tr class="hover:bg-surface-50/70 transition-colors">
                                    ...
                                    <!-- Name -->
                                    <td class="px-4 py-3.5 font-bold text-surface-900 uppercase" x-text="result.participant ? result.participant.full_name : '-'"></td>
                                    ...
                                    <td class="px-4 py-3.5 font-mono text-surface-700" x-text="result.rank_group ? '#' + result.rank_group : '-'"></td>
                                    
                                    <!-- Gender -->
                                    <td class="px-4 py-3.5 uppercase text-surface-600" x-text="result.gender"></td>
                                    ...
                                    <td colspan="10" class="px-6 py-5">
                                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                            ...
                                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-x-8 gap-y-3 text-xs w-full md:w-auto">
                                                <div>
                                                    <span class="text-[9px] text-surface-400 block font-semibold uppercase">Community/Club</span>
                                                    <span class="font-bold text-surface-800" x-text="(result.participant && result.participant.community) || '-'"></span>
                                                </div>
                                                <div>
                                                    <span class="text-[9px] text-surface-400 block font-semibold uppercase">Net Time</span>
                                                    <span class="font-mono font-bold text-surface-800" x-text="result.net_time || '-'"></span>
                                                </div>
                                                <div>
                                                    <span class="text-[9px] text-surface-400 block font-semibold uppercase">Gun Time</span>
                                                    <span class="font-mono font-bold text-surface-800" x-text="result.gun_time || '-'"></span>
                                                </div>
                                                <div>
                                            ...
                                            <div class="flex flex-wrap gap-2 w-full md:w-auto shrink-0 mt-3 md:mt-0">
                                                <button @click="showDetailsModal(result)" class="flex-1 md:flex-none inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white border border-surface-300 hover:border-brand-500 hover:text-brand-600 text-surface-700 font-semibold rounded-xl shadow-sm transition-all text-xs" type="button">
                        ...
                        <tbody x-show="filteredResults.length === 0" x-cloak>
                            <tr>
                                <td colspan="10" class="px-6 py-12 text-center text-surface-400 italic">No results found for your search criteria.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="mt-4 flex justify-between items-center px-4" x-show="activeTab !== 'podium'">
                <p class="text-[9px] uppercase font-bold text-surface-400 tracking-widest">
                    Showing <span x-text="filteredResults.length"></span> runners in <span x-text="activeTab === 'all' ? 'All Categories' : activeTab + 'K'"></span>
                </p>
                <p class="text-[9px] text-surface-300 italic">Official Data Static Cache</p>
            </div>
                                    ...
                                    @foreach($genders as $gender => $winners)
                                        <div class="bg-white rounded-2xl p-6 shadow-sm border border-surface-200">
                                            <div class="flex justify-between items-center mb-6">
                                                ...
                                                <span class="text-[10px] px-2 py-0.5 bg-surface-100 text-surface-500 rounded font-bold uppercase">{{ (int)$distance }}K</span>
                                            </div>
                                            <div class="space-y-4">
                ...
                <div class="relative bg-white rounded-3xl max-w-md w-full overflow-hidden shadow-2xl border border-surface-200"
                    ...
                    <div class="bg-gradient-to-r from-brand-800 to-brand-950 px-6 py-5 text-white flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-[10px] text-white/70" x-text="selectedResult ? 'BIB #' + selectedResult.bib_number : ''"></p>
                        </div>
                    ...
                    <div class="p-6 space-y-6" x-show="selectedResult">
                        <div class="flex items-center gap-4 border-b border-surface-100 pb-4">
                            ...
                            <div>
                                <h4 class="font-bold text-surface-900 text-sm uppercase" x-text="selectedResult && selectedResult.participant ? selectedResult.participant.full_name : ''"></h4>
                                <p class="text-[10px] text-surface-500 font-mono" x-text="selectedResult && selectedResult.participant && selectedResult.participant.community ? 'Club: ' + selectedResult.participant.community : 'No Club / Community'"></p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="bg-surface-50 p-3 rounded-2xl border border-surface-200/60">
                                ...
                                <span class="font-display font-black text-lg text-surface-800" x-text="selectedResult ? selectedResult.age_category + ' (' + (selectedResult.gender === 'male' ? 'M' : 'F') + ')' : '-'"></span>
                            </div>
                            <div class="bg-surface-50 p-3 rounded-2xl border border-surface-200/60">
                                <span class="text-[9px] font-bold text-surface-400 uppercase tracking-wider block mb-0.5">Net Time</span>
                                <span class="font-mono font-bold text-lg text-brand-600" x-text="selectedResult ? selectedResult.net_time || '-' : '-'"></span>
                            </div>
                            <div class="bg-surface-50 p-3 rounded-2xl border border-surface-200/60">
                                <span class="text-[9px] font-bold text-surface-400 uppercase tracking-wider block mb-0.5">Gun Time</span>
                        ...
                        <div class="border-t border-surface-100 pt-4 space-y-2">
                            ...
                            <div class="flex justify-between items-center text-xs py-1 border-b border-surface-50">
                                <span class="text-surface-600" x-text="selectedResult ? 'Group Position (' + selectedResult.age_category + ' ' + selectedResult.gender + ')' : 'Group Position'"></span>
                                <span class="font-mono font-bold text-surface-900" x-text="selectedResult && selectedResult.rank_group ? '#' + selectedResult.rank_group : '-'"></span>
                            </div>
                            <div class="flex justify-between items-center text-xs py-1 border-b border-surface-50">
                                <span class="text-surface-600" x-text="selectedResult ? 'Gender Position (' + selectedResult.gender + ')' : 'Gender Position'"></span>
                                <span class="font-mono font-bold text-surface-900" x-text="selectedResult && selectedResult.gender_pos ? '#' + selectedResult.gender_pos : '-'"></span>
                            </div>
                    ...
                    <div class="bg-surface-50 px-6 py-4 flex justify-end gap-2 border-t border-surface-150">
                        <button @click="showModal = false" class="px-4 py-2 bg-white border border-surface-300 hover:bg-surface-50 text-surface-700 font-semibold rounded-xl text-xs transition-colors">
                            Close
                        </button>
                ...
                <div class="relative bg-white rounded-3xl max-w-2xl w-full overflow-hidden shadow-2xl border border-surface-200"
                    ...
                    <div class="bg-gradient-to-r from-brand-800 to-brand-950 px-6 py-5 text-white flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-[10px] text-white/70">Preview your official event certificate</p>
                        </div>
                    ...
                    <div class="p-6 bg-surface-100 flex justify-center">
                        ...
                        <div id="cert-mockup" class="w-full aspect-[1.414/1] max-w-xl bg-white border-[8px] border-brand-600 p-5 relative flex flex-col justify-between text-center select-none shadow-lg rounded-sm" style="background-image: radial-gradient(circle, #ffffff 0%, #fffafa 100%);">
                            ...
                            <div class="mt-2">
                                <h4 class="text-surface-900 font-display font-black text-sm tracking-wider">ERAFONE TRAIL RUN 2026</h4>
                            ...
                            <div class="my-2">
                                ...
                                <p class="text-surface-500 font-serif italic text-[10px] mt-0.5">This is proudly presented to</p>
                            </div>
                            
                            <div class="my-1">
                                <h2 class="text-surface-900 font-bold text-lg tracking-wide uppercase border-b border-amber-500 inline-block px-6 pb-0.5" x-text="certResult && certResult.participant ? certResult.participant.full_name : '-'"></h2>
                            </div>
                            
                            <p class="text-surface-600 text-[10px] px-6" x-text="'for successfully completing the Erafone Trail Run 2026 in the ' + (certResult ? certResult.distance_km : '-') + 'K Category'"></p>
                            
                            <div class="grid grid-cols-4 bg-surface-50 border border-surface-200 py-2 mt-3 text-center divide-x divide-surface-200">
                                <div>
                                    <span class="text-[7px] text-surface-400 uppercase font-bold block">BIB Number</span>
                                    <span class="text-xs font-bold text-surface-800" x-text="certResult ? certResult.bib_number : '-'"></span>
                                </div>
                                <div>
                                    <span class="text-[7px] text-surface-400 uppercase font-bold block">Category Rank</span>
                                    <span class="text-xs font-bold text-surface-800" x-text="certResult && certResult.rank_category ? '#' + certResult.rank_category : '-'"></span>
                                </div>
                                <div>
                                    <span class="text-[7px] text-surface-400 uppercase font-bold block">Net Time</span>
                                    <span class="text-xs font-bold text-surface-800" x-text="certResult ? certResult.net_time || certResult.gun_time || '-' : '-'"></span>
                                </div>
                                <div>
                                    <span class="text-[7px] text-surface-400 uppercase font-bold block">Avg Pace</span>
                                    <span class="text-xs font-bold text-surface-800" x-text="certResult ? certResult.pace : '-'"></span>
                                </div>
                            </div>
                            
                            <div class="flex justify-between items-end mt-3 text-left px-2">
                                <div>
                                    <p class="text-[8px] text-surface-400 font-mono">Date: 28 June 2026</p>
                                    <p class="text-[8px] text-surface-400">Jakarta, Indonesia</p>
                                </div>
                                <div class="text-center">
                                    <div class="w-16 border-b border-surface-300 mb-0.5"></div>
                                    <p class="text-[7px] text-surface-400">Race Director</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-surface-50 px-6 py-4 flex justify-between items-center border-t border-surface-150">
                        <p class="text-[9px] text-surface-400">High-resolution PNG will be generated on download</p>
                        <div class="flex gap-2">
                            <button @click="showCertModal = false" class="px-4 py-2 bg-white border border-surface-300 hover:bg-surface-50 text-surface-700 font-semibold rounded-xl text-xs transition-colors">
                                Close
                            </button>
