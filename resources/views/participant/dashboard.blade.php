...
<div class="mb-8">
    ...
    <p class="text-surface-700">{{ __('messages.part_welcome') }} <span class="text-brand-500 font-semibold">{{ auth()->user()->name }}</span></p>
</div>
...
<div class="bg-white rounded-2xl border border-surface-300 p-8 text-center shadow-sm">
    ...
    <a href="{{ route('register.create') }}" class="inline-block mt-4 px-6 py-2.5 bg-brand-500 hover:bg-brand-600 font-medium text-white rounded-xl transition-colors shadow-md">{{ __('messages.part_register_now') }}</a>
</div>
@else
    ...
    <div class="md:col-span-2 space-y-6">
        <div class="bg-white rounded-2xl border border-surface-300 overflow-hidden shadow-sm">
            ...
            <div class="p-6 grid grid-cols-2 gap-4">
                <div>
                    <span class="block text-xs text-surface-700 uppercase tracking-wide mb-1">{{ __('messages.part_event') }}</span>
                    <span class="text-surface-900 font-medium">{{ $participant->event->name }}</span>
                </div>
                <div>
                    <span class="block text-xs text-surface-700 uppercase tracking-wide mb-1">{{ __('messages.part_event_date') }}</span>
                    <span class="text-surface-900 font-medium">{{ $participant->event->event_date->format('d M Y') }}</span>
                </div>
                <div>
                    <span class="block text-xs text-surface-700 uppercase tracking-wide mb-1">{{ __('messages.part_event_location') }}</span>
                    <span class="text-surface-900 font-medium">{{ __('messages.hero_location') }}</span>
                </div>
                <div>
                    <span class="block text-xs text-surface-700 uppercase tracking-wide mb-1">{{ __('messages.part_payment_status') }}</span>
                    <span class="text-{{ $participant->payment_status == 'paid' ? 'emerald-600' : ($participant->payment_status == 'pending' ? 'accent-600' : 'brand-500') }} font-medium">{{ ucfirst($participant->payment_status) }}</span>
                </div>
            </div>
        </div>
    </div>
    <div>
        <div class="bg-gradient-to-br from-brand-500 to-brand-600 rounded-2xl border border-brand-400/30 p-6 text-center transform hover:-translate-y-1 transition-transform shadow-lg">
            ...
            <span class="text-xs text-white/60 bg-white/10 px-3 py-1 rounded-full">{{ __('messages.part_bib_pending') }}</span>
            @endif
        </div>
    </div>
</div>
