        ...
        <div class="max-w-md mx-auto relative group">
            @if(!$participant->bib_number)
                <div class="bg-white rounded-2xl border border-surface-300 p-8 text-center shadow-sm">
                    <svg class="w-16 h-16 mx-auto text-accent-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                            d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <h4 class="text-xl font-display font-bold text-surface-900 mb-2">{{ __('messages.part_bib_not_available') }}
                    </h4>
            ...
            @else
                ...
                <div class="bg-white rounded-3xl overflow-hidden shadow-2xl relative">
                    <div class="absolute inset-0 bg-opacity-10 pointer-events-none"
                        style="background-image: radial-gradient(circle at 1px 1px, #e5e5e5 1px, transparent 0); background-size: 20px 20px;">
                    </div>

                    <!-- Header -->
                    <div class="bg-gradient-to-r {{ $catColor }} px-6 py-4 flex justify-between items-center relative z-10">
                        <span class="font-display font-bold text-lg text-white">ERA <span class="text-white/70">TRAIL</span></span>
                        <span
                            class="font-display font-bold text-sm bg-white/20 px-3 py-1 rounded-full text-white backdrop-blur-sm">{{ $participant->category->name }}</span>
                    </div>

                    <!-- BIB Number -->
                    <div class="text-center py-10 relative z-10">
                        <h2 class="font-display font-black text-6xl tracking-widest text-surface-900">{{ $participant->bib_number }}
                        </h2>
                    </div>
                    ...
                    <div class="bg-surface-100 flex items-center justify-between p-6">
                        <div class="flex items-center gap-4">
                            <div class="w-16 h-16 bg-white rounded border border-surface-300 flex items-center justify-center">
                                ...
                                <svg class="w-10 h-10 text-surface-700" viewBox="0 0 24 24" fill="currentColor">
                                    <path
                                        d="M4 4h4v4H4V4zm6 0h10v4H10V4zM4 10h4v10H4V10zm6 0h4v4h-4v-4zm6 0h4v10h-4V10zm-6 6h4v4h-4v-4z" />
                                </svg>
                            </div>
                            <div>
                                <span
                                    class="block text-xs text-surface-700 font-medium pb-0.5">{{ __('messages.part_bib_show_qr') }}</span>
                                <span class="block text-xl font-display font-bold text-surface-800">{{ $distKm }} <span
                                        class="text-sm">{{ __('messages.part_kilometers') }}</span></span>
                            </div>
                        </div>
                        <div
                            class="w-12 h-12 rounded-full border-4 border-white shadow-inner bg-brand-600 flex items-center justify-center">
